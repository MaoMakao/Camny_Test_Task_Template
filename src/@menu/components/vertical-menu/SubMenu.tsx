...
const SubMenu: ForwardRefRenderFunction<HTMLLIElement, SubMenuProps> = (props, ref) => {
  ...
  const handleSlideToggle = (): void => {
    if (level === 0 && isCollapsed && !isHovered && !alwaysActive) {
      return
    }
  ...
  useLayoutEffect(() => {
    if (isCollapsed && !alwaysActive && level === 0) {
      setOpenWhenCollapsed(false)
    }
  ...
  const submenuContent = (
    <SubMenuContent
      ref={isCollapsed && !alwaysActive && level === 0 && isPopoutWhenCollapsed ? refs.setFloating : contentRef}
      {...(isCollapsed && !alwaysActive && level === 0 && isPopoutWhenCollapsed && getFloatingProps())}
      ...
      open={isSubMenuOpen}
      level={level}
      isCollapsed={isCollapsed && !alwaysActive}
      isHovered={isHovered}
      className={classnames(menuClasses.subMenuContent, contentClassName)}
      rootStyles={{
        ...(isCollapsed && !alwaysActive && level === 0 && isPopoutWhenCollapsed && floatingStyles),
  ...
  return (
    ...
    >
      ...
      <MenuButton
        ref={
          isCollapsed && !alwaysActive && level === 0 && isPopoutWhenCollapsed && !disabled ? refs.setReference : null
        }
        onClick={handleOnClick}
        {...(isCollapsed && !alwaysActive && level === 0 && isPopoutWhenCollapsed && !disabled && getReferenceProps())}
      ...
      >
        ...
        {isCollapsed && !alwaysActive && !isHovered && level === 0 ? null : (
          <StyledVerticalNavExpandIconWrapper
      ...
      {/* Sub Menu Content */}
      {isCollapsed && !alwaysActive && level === 0 && isPopoutWhenCollapsed ? (
